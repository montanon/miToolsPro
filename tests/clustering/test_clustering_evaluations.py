...
from mitoolspro.clustering import (
...
from mitoolspro.clustering.clustering_evaluations import N_ELEMENTS_COL
...
class TestGetClustersSize(TestCase):
    def setUp(self):
        self.data = DataFrame(
...
class TestGetCosineSimilaritiesVector(TestCase):
    def setUp(self):
        index = pd.MultiIndex.from_tuples(
            [("A", 1), ("A", 2), ("A", 3), ("B", 4), ("B", 5), ("B", 6)],
            names=["cluster", "sample_id"],
    ...
    def test_positive_case(self):
        result = get_cosine_similarities_vector(self.data, id_level="sample_id")
        similarity_matrix = cosine_similarity(self.data.values)
        upper_tri_indices = np.triu_indices_from(similarity_matrix, k=1)
        sample_pairs = [
            (
    ...
    def test_empty_dataframe(self):
        ...
        with self.assertRaises(ArgumentStructureError):
            get_cosine_similarities_vector(empty_data)

    def test_single_row_dataframe(self):
        single_row_df = self.data.iloc[[0]]
        with self.assertRaises(ArgumentStructureError):
            get_cosine_similarities_vector(single_row_df)
...
class TestGetCosineSimilarities(TestCase):
    ...
    def test_as_vector(self):
        result = get_cosine_similarities(
            self.data, id_level="sample_id", as_vector=True
        )
        similarity_matrix = cosine_similarity(self.data.values)
        upper_tri_indices = np.triu_indices_from(similarity_matrix, k=1)
        sample_pairs = [
    ...
    def test_as_matrix(self):
        result = get_cosine_similarities(
            self.data, id_level="sample_id", as_vector=False
        )
        expected_result = DataFrame(
            cosine_similarity(self.data.values),
            index=self.data.index.get_level_values("sample_id"),
...
class TestGetCosineSimilaritiesMatrix(TestCase):
    def setUp(self):
        index = pd.MultiIndex.from_tuples(
            [("A", 1), ("A", 2), ("A", 3), ("B", 4), ("B", 5), ("B", 6)],
            names=["cluster", "sample_id"],
    ...
    def test_with_id_level(self):
        result = get_cosine_similarities_matrix(self.data, id_level="sample_id")
        expected_result = DataFrame(
            cosine_similarity(self.data.values),
            index=self.data.index.get_level_values("sample_id"),
            columns=self.data.index.get_level_values("sample_id"),
    ...
    def test_with_numeric_id_level(self):
        result = get_cosine_similarities_matrix(self.data, id_level=1)
        expected_result = DataFrame(
            cosine_similarity(self.data.values),
            index=self.data.index.get_level_values(1),
            columns=self.data.index.get_level_values(1),
    ...
    def test_invalid_id_level(self):
        with self.assertRaises(KeyError):
            get_cosine_similarities_matrix(self.data, id_level="invalid_level")
